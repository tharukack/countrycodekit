...
class ShortNumberInfo internal constructor(// MatcherApi supports the basic matching method for checking if a given national number matches
     ...
     */
    private fun getShortNumberMetadataForRegion(regionCode: String?): PhoneMetadata? {
        return if (regionCode == null) {
            null
        } else try {
            shortNumberMetadataSource.getMetadataForRegion(regionCode)
    ...
    fun getExpectedCost(number: PhoneNumber): ShortNumberCost {
        ...
        for (regionCode in regionCodes) {
            ...
            when (costForRegion) {
                ...
                ShortNumberCost.TOLL_FREE -> {}
                else -> logger.e("Unrecognised cost for region: $costForRegion")
            }
        }
        return cost
    }
    ...
    companion object {
        private val logger = Logger.withTag(ShortNumberInfo::class.simpleName.toString())
